...
export function PaginatedRepositoryIssues(props) {
  ...
  const postUses = repositoryForPaginatedIssues?.posts?.edges?.map((item, idx) => <Post key={item?.node?.id || idx} issue={item?.node} />);
    ...
    <div>
      ...
      <button
        ...
        onClick={() => {
          if (!relay.isLoading()) {
            setIsLoading(true);
            relay.loadMore(loadMoreCount, (result) => {
...
export const PaginatedRepositoryIssuesContainer = createPaginationContainer(
  ...
  id
  posts: issues(first: $count, orderBy: {field: CREATED_AT, direction: DESC}, filterBy: {createdBy: $createdBy}, after: $cursor) @connection(key: "Posts_repositoryForPaginatedIssues_posts") {
    edges {
      node {
        ...Post_fragment
      }
...
const POSTS_QUERY = graphql`
  query PostsQuery($owner: String = "onegraph", $name: String = "essay.dev", $createdBy: String = "sgrove") {
    gitHub {
...
export function PostsQuery(props) {
  ...
  return (
    <div>
      <h4>PaginatedRepositoryIssuesUses <LocationNode /></h4>
