...
export function PaginatedRepositoryIssues(props) {
  ...
  const issuesForPaginatedCommentsUses = repositoryForPaginatedIssues?.issues?.edges?.map((item, idx) => <PaginatedIssuesCommentsContainer key={item?.node?.id || idx} issuesForPaginatedComments={item?.node} />);
...
export const PaginatedRepositoryIssuesContainer = createPaginationContainer(
  ...
  id
  issues(first: $count, orderBy: {field: CREATED_AT, direction: DESC}, labels: $labels, filterBy: {createdBy: $createdBy}, after: $cursor) @connection(key: "Posts_repositoryForPaginatedIssues_issues") {
    edges {
      node {
        id
        number
